...
class SubsetSubcubeDataModule(L.LightningDataModule):
    """Defines access to the Spitzer hdf5 data as a data module."""
    def __init__(
        self,
        data_directories: List[str],
        ...
        flip: float = 0.5,
        crop_size: int = 16,
        loss_gate: float = 0.002,
        loss_indx: str = 'losses.npy'
    ):
        """Inititalize the data loader for the Spitzer data.
    ...
    def setup(self, stage: str):
        ...
        if stage == "fit":
            ...
            self.data_train = Subset(
                ...
                indices=np.argwhere(np.load(self.loss_indx) > self.loss_gate)
            )
